...
```{r}
# ✅ Load clinical data (already done)
clinical_data <- readRDS("TCGA_LUAD_Clinical.rds")

# ✅ Convert patient ID (12-character TCGA barcode)
clinical_data$sample_id <- substr(clinical_data$submitter_id, 1, 12)

# ✅ Standardize and compute survival time and event
...
# ✅ Summary
cat("✅ Cleaned clinical samples with survival data:", nrow(clinical_data_clean), "\n")
...
# Get mapping
entrez_ids <- deg_filtered$gene[1:30]
...
print(mapped_symbols)  # Check output
```

```{r}
# mapped_symbols is your vector of gene symbols (some NAs)
mapped_symbols_clean <- na.omit(mapped_symbols)
mapped_symbols_clean <- mapped_symbols_clean[mapped_symbols_clean %in% rownames(expr_matrix_qc)]
library(survival)
results_surv <- list()

for (gene in mapped_symbols_clean) {
  cat("Processing gene:", gene, "\n")
  try({
    expr_values <- expr_matrix_qc[gene, clinical_data_clean$sample_id]
    clinical_data_clean$gene_expr <- expr_values
    
    fit <- coxph(Surv(surv_time, event) ~ gene_expr + gender, data = clinical_data_clean)
    results_surv[[gene]] <- summary(fit)
    cat("  Success: Model fitted for", gene, "\n")
  }, silent = TRUE)
}
...
```{r}
all(mapped_symbols_clean %in% rownames(expr_matrix_qc))
all(clinical_data_clean$sample_id %in% colnames(expr_matrix_qc))
```
```{r}

convert_id <- function(id) {
  # Convert "TCGA-05-4430" to "tcga_05_4430"
  id <- tolower(id)
  id <- gsub("-", "_", id)
  return(id)
}

clinical_data_clean$expr_id <- convert_id(clinical_data_clean$sample_id)
print(head(clinical_data_clean$sample_id))
```

```{r}
print(head(clinical_data_clean$sample_id))
print(head(clinical_data_clean$expr_id))
print(head(colnames(expr_matrix_qc)))
```
```{r}
expr_matrix_base_ids <- sub("(_[0-9][0-9])$", "", colnames(expr_matrix_qc))
...
clinical_data_matched <- clinical_data_clean[clinical_data_clean$expr_id %in% matched_expr_ids, ]
# Reorder so expr_id and expr_matrix columns match
clinical_data_matched <- clinical_data_matched[match(matched_expr_ids, clinical_data_matched$expr_id), ]
cat("Number of matched samples:", length(matched_samples), "\n")
print(head(matched_samples))
print(head(clinical_data_matched$expr_id))
```
...
# Filter and order clinical data to match the expression matrix
...
for (gene in mapped_symbols_clean) {
  expr_values <- expr_matrix_qc[gene, matched_samples]
  df <- clinical_data_matched
  df$gene_expr <- expr_values

  # Remove any rows with missing data in gene_expr, surv_time, or event
  df <- df[!is.na(df$gene_expr) & !is.na(df$surv_time) & !is.na(df$event), ]

  # Skip gene if no valid samples or zero variance
  if (nrow(df) == 0 || var(df$gene_expr, na.rm = TRUE) == 0) {
    cat("Skipping gene due to missing/zero-variance expression:", gene, "\n")
    next
  }

  fit <- try(coxph(Surv(surv_time, event) ~ gene_expr + gender, data = df), silent = TRUE)
  if (!inherits(fit, "try-error")) {
    results_surv[[gene]] <- summary(fit)
    cat("Model fitted for gene:", gene, "\n")
  } else {
    cat("Error fitting model for gene:", gene, "\n")
  }
}

cat("Genes with fitted models:\n")
print(names(results_surv))
```
...
for (gene in names(results_surv)) {
  s <- results_surv[[gene]]
  # Assumes gene_expr is the first variable in the model
  hr <- s$coefficients["gene_expr", "exp(coef)"]
  pval <- s$coefficients["gene_expr", "Pr(>|z|)"]
  results <- rbind(results, data.frame(gene = gene, HR = hr, p.value = pval))
}
...
```
```{r}
# 🔧 Clean colnames in expr_matrix_qc
...
```{r}
is_numeric_col <- sapply(clinical_df_tidy[, num_cols], function(x) all(!is.na(as.numeric(as.character(x)))))
